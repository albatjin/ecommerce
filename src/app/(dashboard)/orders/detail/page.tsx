...
export default async function OrderDetailDefaultPage() {
  const supabase = await createClient();
  const repository = new SupabaseOrderRepository(supabase);
  const getOrderDetail = new GetOrderDetailUseCase(repository);

  // Load sample default order 'ord-01' (ORD-20250520-00192)
  const order = await getOrderDetail.execute('ord-01');
